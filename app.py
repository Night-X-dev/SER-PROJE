...
def ik_login():
    try:
        ...
        try:
            ...
            print(f"Password encoding error: {e}")  # Debug log
            return jsonify({'success': False, 'message': 'Geçersiz şifre formatı'}), 400
        ...
        with connection.cursor() as cursor:
            ...
            # Verify password
            if bcrypt.checkpw(password, user['sifre'].encode('utf-8')):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                (ad, soyad, email, sifre, telefon, departman, pozisyon, ise_baslama_tarihi, durum, kayit_tarihi)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'pasif', NOW())
                """,
                (
                    data['ad'],
                    data['soyad'],
                    ...
                    data.get('departman', ''),
                    data.get('pozisyon', ''),
                    data.get('ise_baslama_tarihi') or None
                )
            )
            connection.commit()
            ...
            return jsonify({
                'success': True, 
                'message': 'Kayıt başarılı. Hesabınız yönetici onayından sonra aktif hale gelecektir.'
            })
