...
def complete_progress_step(progress_id):
    try:
        ...
        connection = get_db_connection()
        with connection.cursor() as cursor:
            # İş adımını tamamlandı olarak işaretle
            cursor.execute("""
                UPDATE project_progress 
                SET status = %s, 
                    real_end_date = CASE WHEN %s = TRUE THEN CURDATE() ELSE NULL END
                WHERE progress_id = %s
            """, ('tamamlandi' if completed else 'devam_ediyor', completed, progress_id))
            
            # Projenin durumunu güncelle
            cursor.execute("""
                SELECT project_id FROM project_progress WHERE progress_id = %s
            """, (progress_id,))
            project_id = cursor.fetchone()['project_id']
            
            # Projenin tüm adımlarının durumunu kontrol et
            cursor.execute("""
            ...
            # Tüm adımlar tamamlandıysa projeyi de tamamlandı olarak işaretle
            ...
            # E-posta gönder
            try:
                # İş adımı bilgilerini al
                cursor.execute("""
                    SELECT pp.title, p.project_name, u.email, u.fullname
            ...
            return jsonify({'message': 'İş adımı durumu güncellendi'}), 200
...
def send_completion_email(to_email, manager_name, project_name, step_title):
    try:
        smtp_server = "smtp.gmail.com"
        smtp_port = 587
        sender_email = "your-email@gmail.com"  # E-posta adresiniz
        sender_password = "your-password"  # Uygulama şifreniz

        message = MIMEMultipart("alternative")
        message["Subject"] = f"İş Gidişatı Tamamlandı: {step_title}"
        message["From"] = sender_email
        message["To"] = to_email
