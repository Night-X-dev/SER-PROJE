...
def update_project_progress_step(progress_id):
    """
    Updates a project progress step and recalculates delays for subsequent steps.
    The logic has been updated to include a delay when a step starts on the
    same day as the previous step ends.
    """
    data = request.get_json()
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # Get the real end date from the database to preserve it
            ...
            # If a real_end_date exists, use it; otherwise, use the new end_date
            ...
            # Get project name and manager ID for notifications
            ...
            # Recalculate calculated_delay_days based on the previous step's end date
            ...
            if previous_step and previous_step['end_date']:
                ...
                # A delay is calculated if the current step starts on the same day as or
                # before the previous step's end date.
                # A 1-day gap (i.e., starting the day after) is considered on-time.
                if time_diff < 1:
                    calculated_delay_days = 1 - time_diff
                # --- MODIFIED LOGIC END ---
            elif not previous_step: # If this is the first step, calculate delay based on project start date
            ...
            # Recalculate and update the delay_days for subsequent steps
            cursor.execute("""
            ...
            last_end_date_for_recalc = datetime.datetime.strptime(end_date_str, '%Y-%m-%d').date() # Use the new end date of the current step
            ...
            for sub_step in subsequent_steps:
                ...
                # before the previous step's end date.
                if time_diff_sub < 1:
                    recalculated_sub_delay_days = 1 - time_diff_sub
                # --- MODIFIED LOGIC END ---

                # Update the real end date of the next step (preserve the existing one if it's there)
                ...
                last_end_date_for_recalc = sub_end_date # Update end date for the next iteration
