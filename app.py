...
def mark_all_notifications_as_read():
    ...
    if not user_id:
        return jsonify({'message': 'Kullanıcı ID zorunludur.'}), 400

    connection = get_db_connection()
    try:
        with connection.cursor() as cursor:
            sql = "UPDATE notifications SET is_read = 1 WHERE is_read = 0 AND user_id = %s"
...
def get_unread_notifications_count():
    ...
    try:
        with connection.cursor() as cursor:
            sql = "SELECT COUNT(id) as unread_count FROM notifications WHERE is_read = 0 AND user_id = %s"
            cursor.execute(sql, (user_id,))
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if not cursor.fetchone():
                return jsonify({'message': 'Bildirim bulunamadı veya bu kullanıcıya ait değil.'}), 404
...
def get_notifications():
    ...
    user_id = request.args.get('user_id') # Query parametresi olarak user_id alınır
...
def add_notification_api(): # Renamed to avoid conflict with helper function
    ...
    try:
        ...
        with connection.cursor() as cursor:
            sql = """
            INSERT INTO notifications (user_id, title, message, icon, created_at)
            VALUES (%s, %s, %s, %s, NOW())
            """
            cursor.execute(sql, (user_id, title, message, icon))
            connection.commit()
...
def delete_user(user_id):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            print(f"DEBUG: Deleted {cursor.rowcount} notifications for user {user_id}")
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if new_project_manager_id is not None and new_project_manager_id != old_project_manager_id:
                ...
                # Yeni proje yöneticisine bildirim gönder
                log_notification(
...
def add_task():
    ...
    created_by_user_id = data.get('created_by') # Görevi oluşturan kullanıcı ID'si

    if not all([title, start_date, assigned_user_id, created_by_user_id]):
        return jsonify({'message': 'Başlık, başlangıç tarihi, atanan kullanıcı ve oluşturan kullanıcı zorunludur.'}), 400
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s)
            """
            cursor.execute(sql, (title, description, start_date, end_date, priority, assigned_user_id, created_by_user_id))
            connection.commit()
            ...
            log_notification(
                ...
                message=f'"{title}" görevi size atandı.',
                icon='fas fa-tasks'
            )
        return jsonify({'message': 'Görev başarıyla eklendi!'}), 201
    except pymysql.Error as e:
        print(f"Veritabanı görev ekleme hatası: {e}")
        return jsonify({'message': f'Veritabanı hatası oluştu: {e.args[1]}'}), 500
    except Exception as e:
        print(f"Genel görev ekleme hatası: {e}")
        return jsonify({'message': 'Sunucu hatası, lütfen daha sonra tekrar deneyin.'}), 500
    finally:
        if connection:
            connection.close()
...
def update_task(task_id):
    ...
    current_user_id = data.get('current_user_id') # Güncellemeyi yapan kullanıcı

    if not all([title, start, assigned_user_id, created_by, current_user_id]):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            old_assigned_user_id = existing_task['assigned_user_id']
            ...
            # Görev başlığı veya atanan kişi değiştiyse bildirim gönder
            if title != old_title or assigned_user_id != old_assigned_user_id:
                # Eğer atanan kişi değiştiyse, eski atanana görev iptali bildirimi
